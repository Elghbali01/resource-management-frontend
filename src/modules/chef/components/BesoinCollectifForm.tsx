...
export default function BesoinCollectifForm({ demandeId, onSubmitted, besoinToEdit, onCanceledEdit }: Props) {
  ...
  useEffect(() => {
    if (besoinToEdit) {
      ...
      setForm((prev) => ({ ...prev, demandeId })); // sync
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (besoinToEdit) {
        ...
        onSubmitted(created);
        setSuccess("Besoin collectif soumis avec succès.");
        setForm({
          demandeId,
          typeMateriel: "ORDINATEUR",
          quantite: 1,
      ...
      const apiMessage =
        ...
        "Erreur lors de l'enregistrement du besoin collectif";
      setError(apiMessage);
    } finally {
      setLoading(false);
    }
      ...
      <form onSubmit={handleSubmit} className="collecte-form p-4 border border-gray-100 shadow-sm mt-4 space-y-4">
        ...
        <div className="flex gap-4">
          <div className="flex-1">
            ...
            <select
              ...
              className="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:ring focus:border-blue-300 bg-white"
            >
        ...
        {form.typeMateriel === "ORDINATEUR" && (
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" name="cpu" value={form.cpu} onChange={handleChange} placeholder="Ex: i7" className="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:ring focus:border-blue-300" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">RAM</label>
              <input type="text" name="ram" value={form.ram} onChange={handleChange} placeholder="Ex: 16 Go" className="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:ring focus:border-blue-300" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Disque dur</label>
              <input type="text" name="disqueDur" value={form.disqueDur} onChange={handleChange} placeholder="Ex: 512 Go SSD" className="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:ring focus:border-blue-300" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Écran</label>
              <input type="text" name="ecran" value={form.ecran} onChange={handleChange} placeholder="Ex: 15 pouces" className="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:ring focus:border-blue-300" />
            </div>
          </div>
        )}
        ...
        {form.typeMateriel === "IMPRIMANTE" && (
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" name="vitesseImpression" value={form.vitesseImpression} onChange={handleChange} placeholder="Ex: 20 ppm" className="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:ring focus:border-blue-300" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Résolution</label>
              <input type="text" name="resolution" value={form.resolution} onChange={handleChange} placeholder="Ex: 1200 dpi" className="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:ring focus:border-blue-300" />
            </div>
          </div>
        )}
        ...
        <div className="flex justify-end gap-3 pt-2">
          ...
          <button
            type="submit"
            disabled={loading}
            className="collecte-btn-primary"
          >
